...
export function OnboardingWizard({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-ink-950">
      ...
      <header
        className="sticky top-0 z-20 bg-ink-950 px-4 pb-3"
        style={{ paddingTop: 'calc(env(safe-area-inset-top) + 16px)' }}
      >
        <div className="mx-auto max-w-lg">
          <div className="mb-3 flex gap-1" role="progressbar" aria-valuenow={wizard.index + 1} aria-valuemax={wizard.total}>
            {Array.from({ length: wizard.total }, (_, i) => (
              <span
                key={i}
                className={`h-1 flex-1 rounded-full transition-colors ${
                  i < wizard.index ? 'bg-volt-500' : i === wizard.index ? 'bg-volt-400' : 'bg-ink-700'
                }`}
              />
            ))}
          </div>
          <div className="flex items-center justify-between">
            <button
              ...
              className="tap -ml-2 flex h-10 w-10 items-center justify-center rounded-lg text-chalk-300 disabled:opacity-0"
              aria-label="Назад"
            >
              <Icon name="chevronLeft" size={22} />
            </button>
            <span className="text-[11px] font-semibold tracking-[0.12em] text-chalk-500 uppercase">
              Стъпка {wizard.index + 1} от {wizard.total}
            </span>
            <span className="w-10" />
          </div>
        </div>
